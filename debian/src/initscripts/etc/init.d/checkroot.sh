...
do_start() {
	...
	# Linux.
	KERNEL="$(uname)"
	if [ "$rootcheck" = yes ] && [ "$KERNEL" = Linux ]; then
		ddev="$(mountpoint -qx $rootdev)"
		rdev="$(mountpoint -d /)"
		if [ "$ddev" != "$rdev" ] && [ "$ddev" != "4:0" ]; then
			if [ "$(mountpoint -qx /dev/root)" = "4:0" ]; then
				rootdev=/dev/root
			else
				if
					rm -f /dev/rootdev &&
...
Will restart in 5 seconds."
	...
	if [ "$rootcheck" = yes ]; then
		...
		if [ "${KERNEL}:${MACHINE}" = Linux:s390x ]; then
			spinner=""
		fi
...
but requested that the system be restarted."
	...
	#
	if ! mount -n -o remount,$rootopts,$rootmode $fstabroot / 2>/dev/null; then
